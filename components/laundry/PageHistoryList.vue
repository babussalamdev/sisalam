  ...
  <div class="list pb-5">
    ...
    <div v-for="(data, i) in filterData" :key="i" class="d-flex justify-content-between mb-3">
      <div class="d-flex">
        <div
          :class="`historylist ${
            data.GSIPK1 === 'debit' ? 'bg-success' : data.GSIPK1 === 'credit' ? 'bg-danger' : 'bg-primary'
          } d-flex align-items-center justify-content-center me-3`">
          <i
            :class="`bi ${
              data.GSIPK1 === 'debit' ? 'bi-coin' : data.GSIPK1 === 'credit' ? 'bi-shop' : 'bi-credit-card-2-front'
...
<script>
  ...
  export default {
    // Inside PageHistoryList.vue script
    computed: {
      ...mapState("laundry", ["laundryLog"]),
      filterData() {
        const data = this.laundryLog || []; // Guard against null
        // If we are on the full history page, show everything retrieved by the API
