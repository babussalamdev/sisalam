  ...
  <div>
    <div class="p-3 widgets">
      <div class="card p-3 bg-light border-0 shadow-sm rounded-4">
        ...
        <h1 class="mb-1 text-capitalize">{{ absensi?.status }}<i class="material-icons ms-1">
            school </i></h1>
      ...
      <div class="card p-3 bg-success-subtle border-0 shadow-sm rounded-4">
        <p class="mb-2"><i class="bi bi-droplet-fill me-1"></i>Tagihan Laundry</p>
        <h1 class="mb-1">{{ rupiah(50000) }}</h1>
        <h6 class="text-secondary mb-3">Total 25 Pakaian
        </h6>
        <p class="mb-0">{{ pelanggaran.time !== "-" ? changeDate(pelanggaran?.time?.split(' ')[0]) : '-' }}
        </p>
      </div>
